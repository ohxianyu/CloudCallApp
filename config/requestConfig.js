...
let $http = new request({
	...
	header: {
		...
		// "Access-Control-Allow-Headers":"Content-Type, Access-Control-Allow-Headers, Authorization, X-Requested-With, token, access_token",
...
$http.dataFactory = async function(res) {
	...
	if (res.response.statusCode && res.response.statusCode == 200) {
		...
		//判断数据是否请求成功
		if (httpData.success || httpData.code == 1) {
			// 返回正确的结果(then接受数据)
